...
@interface ZLChineseToPinyin()
// 用将需要排序的对象封装在ZLChineseToPinyin对象中，包含排序的字符串，对象，首字母三个属性
// 进行比较的字符串
...
@implementation ZLChineseToPinyin
...
+ (NSMutableArray *)indexArray:(NSArray *)stringArr {
    ...
    for (NSString* object in tempArray) {
        NSString *pinyin = [((ZLChineseToPinyin*)object).pinYin substringToIndex:1];
        // 不同
...
+ (NSMutableArray *)letterSortArray:(NSArray *)stringArr {
    ...
    for (NSString *object in tempArray) {
        ...
        NSString *pinyin = [((ZLChineseToPinyin*)object).pinYin substringToIndex:1];
        NSString *string = ((ZLChineseToPinyin*)object).string;
...
+ (NSMutableArray *)returnSortChineseArrar:(NSArray *)stringArr {
    ...
    for(int i = 0; i < [stringArr count]; i++) {
        ...
        ZLChineseToPinyin *chineseString = [[ZLChineseToPinyin alloc]init];
        ...
        chineseString.string = [ZLChineseToPinyin removeSpecialCharacter:chineseString.string];
...
+ (NSMutableArray *)sortObjectArray:(NSArray *)objectArray Key:(NSString *)key {
    ...
    NSString *tempString;
    // 拼音分组
    for (ZLChineseToPinyin* object in tempArray) {
    ...
    if (ZLLog == 1) { // 1、打印函数耗时   0、不打印
        ...
        NSLog(@"ZLChineseToPinyin user time in %f ms", useTime *1000.0);
    }
